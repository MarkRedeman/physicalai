...
class KinematicEndEffectorRobot(Robot):
    ...
    def send_action(self, action: np.ndarray, *, goal_time: float = 0.1) -> None:
        ...
        Raises:
            ValueError: If the action is too short, quaternion is invalid, or IK
                fails to converge.
        """
    ...
    def _inverse_kinematics(self, target: object, seed: np.ndarray) -> np.ndarray:
        ...
        for _ in range(self._ik_options.max_iterations):
            ...
            if np.linalg.norm(error) < self._ik_options.tolerance:
                return q
        ...
        msg = f"IK failed to converge for frame {self._end_effector_frame!r}."
        raise ValueError(msg)
