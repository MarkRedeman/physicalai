...
def validate_robot_payload_ui(payload_model: type[BaseModel]) -> None:
    """Validate Studio UI metadata emitted by a robot payload model."""
    schema = payload_model.model_json_schema()
    definitions = schema.get("$defs", {})

    def error(path: str, message: str) -> None:
        raise ValueError(f"{payload_model.__name__} UI schema at {path}: {message}")
    ...
    def validate_items(
        items: list[object], properties: dict[str, Any], path: str, owned_fields: set[str] | None = None
    ) -> None:
